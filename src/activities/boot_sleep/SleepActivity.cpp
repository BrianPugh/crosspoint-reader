...
void SleepActivity::onEnter() {
  ...
  // The "Entering sleep" popup is painted lazily by the slow render paths
  // (paintGoingToSleepPopupOnce) rather than unconditionally here, so a
  // cache-hit sleep entry avoids the extra full-frame refresh.
...
void SleepActivity::renderCustomSleepScreen() const {
  ...
  // Scan cost scales with the file count: selectRandomSleepFile parses each
  // candidate BMP's headers from SD on every sleep.
  ...
  if (!selectedPath.empty()) {
    ...
    if (Storage.openFileForRead("SLP", selectedPath, randFile)) {
      ...
      const auto cacheKey = SleepArtCache::keyFor(randFile, selectedPath);
      if (SleepArtCache::tryRender(renderer, cacheKey)) {
        randFile.close();
        return;
      }
...
void SleepActivity::renderBitmapSleepScreen(const Bitmap& bitmap, const SleepArtCache::Key* cacheKey,
...
bool SleepActivity::renderSleepOverlayFile(HalFile& file, const char* pathForLog) const {
  ...
  renderBitmapSleepScreen(bitmap, nullptr, /*preserveBackground=*/true);
  return true;
}
